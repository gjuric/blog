...
//$('#search').val(query.get());
el = document.getElementById('search').value = query.get();
...
request.onreadystatechange = function() {
  if (this.readyState === 4) {
    if (this.status >= 200 && this.status < 400) {
      ...
      var db = data.reduce(function (acc, data) {
          acc[data.link] = data
          return acc
      }, {})
      ...
      var results = idx.search(query.get())


  //$('#s').text(results.length + (results.length === 1 ? ' result' : ' results') + ' for "' + query.get() +'"');
      el = document.getElementById('s');
      if (el.textContent !== undefined)
        el.textContent = results.length + (results.length === 1 ? ' result' : ' results') + ' for "' + query.get() +'"';
      else
        el.innerText = results.length + (results.length === 1 ? ' result' : ' results') + ' for "' + query.get() +'"';
...
//    $('#result_list').append(
//      $('<li>').append(
//        $('<a>').attr('href',db[result.ref].link).append(
//          db[result.ref].title)
//    )); 

        var li = document.createElement('li');
        var a = document.createElement('a');
        a.setAttribute('href', db[result.ref].link);
        if (a.textContent !== undefined)
          a.textContent = db[result.ref].title;
        else
          a.innerText = db[result.ref].title;
